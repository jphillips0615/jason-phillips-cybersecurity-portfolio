...
const projects: Project[] = [
  {
    ...
    description: 'Built a small enterprise-style Windows network and tested selected security controls using Windows Server 2025 and Windows 11 in Oracle VirtualBox. The environment demonstrates centralized authentication, DNS, role-based access control, secure file sharing, source-specific firewall rules, Nmap-based exposure assessment, targeted hardening, and before/after validation.',
    ...
    status: 'VALIDATED CONTROLS // LAB EXPANSION IN PROGRESS',
    featured: true,
    ...
    <main className="site-shell min-h-screen overflow-x-hidden text-cyber-text">
      ...
      <section id="projects" className="section-wrap">
        <div className="section-inner">
          <SectionHeading label="01 // PROJECTS" title="Selected Work" text="A documented Windows security lab and two software projects. Each case study explains the implementation, evidence, and current limitations." />
          ...
          <details className="panel-card mt-8">
            <summary className="cursor-pointer text-base font-semibold text-white">Future lab roadmap · planned work</summary>
            <p className="mt-4 leading-7 text-cyber-secondary">These ideas are planned work. Completed evidence will be added as each project is built and tested.</p>
            <div className="mt-5 grid gap-5 md:grid-cols-2">
              {projects.filter((p) => !p.caseStudy).map((p) => (
                <div key={p.title}>
                  <h3 className="text-lg font-semibold text-white">{p.title}</h3>
                  <p className="mt-2 text-base leading-7 text-cyber-secondary">{p.description}</p>
                </div>
              ))}
            </div>
          </details>
      ...
      <section id="about" className="section-wrap">
        <div className="section-inner grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-start">
          <div>
            <SectionHeading label="02 // ABOUT" title="About Jason" />
          ...
          <RevealCard className="panel-card lg:mt-10">
            ...
            <div className="mt-6 space-y-4 font-mono text-xs sm:text-sm">
              {[
                ['NAME','Jason Phillips'],['FIELD','Cybersecurity'],['STATUS','Seeking entry-level opportunities'],['PRIMARY OS','Linux / Windows'],['FOCUS','Security operations / Network security'],['CURRENT OBJECTIVE','Cybersecurity / Security Operations Role'],
      ...
      <section id="skills" className="section-wrap">
        <div className="section-inner">
          <SectionHeading label="03 // CAPABILITIES" title="Technical Skills" text="A working toolkit built through coursework, labs, projects, troubleshooting, and continuous hands-on practice." />
      ...
      <section id="labs" className="section-wrap">
        <div className="section-inner">
          <SectionHeading label="04 // LAB" title="Cyber Range" text="Hands-on environments where concepts become muscle memory and mistakes turn into troubleshooting experience." />
          <div className="grid gap-4 lg:grid-cols-2">
            {labs.map(({ title, icon: Icon, text, bullets }) => (
              <RevealCard key={title} className="panel-card lab-card">
                <div className="flex items-start justify-between gap-4"><div className="icon-box"><Icon size={18}/></div><div className="status-chip"><span className="status-dot"/> HANDS-ON PRACTICE</div></div>
      ...
      <section className="section-wrap">
        <div className="section-inner">
          <SectionHeading label="05 // TRAINING" title="Practical Training" text="Practical training is presented separately from employment so the portfolio stays accurate about where experience comes from." />
      ...
      <section id="experience" className="section-wrap">
        <div className="section-inner">
          <SectionHeading label="08 // EXPERIENCE" title="Professional Experience" text="Cybersecurity is the destination; leadership, operations, accountability, and problem solving are experience I already bring with me." />
          <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="mt-6 rounded-3xl border border-cyber-red/20 bg-cyber-red/[.035] p-6 font-mono text-xs leading-6 text-cyber-secondary">TRANSFERABLE_SIGNAL <span className="text-cyber-red">//</span> Leadership + prioritization + communication + process discipline + decision making under pressure.</div>
        </div>
      </section>
      ...
      <section className="section-wrap">
        <div className="section-inner grid gap-10 lg:grid-cols-[.72fr_1.28fr] lg:items-start">
          <div>
            <SectionHeading label="09 // TERMINAL" title="Command Line" text="Optional keyboard-based portfolio navigation. Type help to see available commands." />
      ...
      <section id="contact" className="section-wrap">
        <div className="section-inner">
          <SectionHeading label="10 // CONNECT" title="Open a Connection" text="Seeking cybersecurity internships and entry-level security operations or IT support roles." />
          <div className="grid gap-6 lg:grid-cols-[.72fr_1.28fr]">
            <div className="space-y-4">
              ...
              <div className="rounded-3xl border border-white/10 bg-cyber-card p-5 font-mono text-xs leading-6 text-cyber-secondary"><span className="text-cyber-red">STATUS //</span> Available to discuss cybersecurity internships and entry-level SOC analyst or IT support opportunities.</div>
            </div>
            <ContactForm />
          </div>
        </div>

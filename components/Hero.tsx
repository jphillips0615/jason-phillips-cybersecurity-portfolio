  ...
  return (
    <section id="home" className="relative flex min-h-screen items-center overflow-hidden pt-24">
      ...
      <div className="relative mx-auto grid w-full max-w-7xl gap-14 px-5 py-16 lg:grid-cols-[1.12fr_.88fr] lg:items-center lg:px-8">
        <div>
          <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-5 font-mono text-xs uppercase tracking-[0.28em] text-cyber-red">CYBERSECURITY STUDENT // HANDS-ON LABS</motion.p>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.26 }} className="mt-7 max-w-2xl text-base leading-7 text-cyber-secondary sm:text-lg">Cybersecurity student at Full Sail University with hands-on experience in Active Directory, Windows network security, and web application testing. I build and document labs, validate security controls, and develop full-stack applications.</motion.p>

          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.36 }} className="mt-7 inline-flex min-h-12 py-3 items-center rounded-xl border border-white/10 bg-black/55 px-4 font-mono text-xs text-cyber-secondary sm:text-sm">
            <span className="text-white">Seeking cybersecurity internships and entry-level SOC / IT support roles</span>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.44 }} className="mt-8 flex flex-wrap gap-3">
            ...
            <a href="/Jason_Phillips_Resume.pdf" download className="btn-secondary">RESUME <Download size={16} /></a>
      ...
      <a href="#about" className="absolute bottom-6 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-2 font-mono text-[9px] tracking-[0.24em] text-cyber-muted md:flex">
        ABOUT JASON <ArrowDown size={14} className="scroll-arrow" />
      </a>
    </section>
  )
}
